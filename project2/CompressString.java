...
public class CompressString {
	...
	/**
	 * Iterative algorithm to resolve this problem. This algorithm has a complexity
	 * order in time and space terms equals to O(N) where N is the number of chars
	 * in the input String. We've applied two guards, one for null strings and other
	 * for input data with 0 or 1 char. The algorithm is based on two pointers to
	 * the current and the previous char in the input string and one counter to take
	 * the count of how many times the char is inside the word. Whe the algorithm
	 * detects a char change, it writes in the StringBuilder the compressed partial
	 * result.
	 */
	...
	/**
	 * Time complexity O(n) Space complexity O(n) First while loop for finding
	...
	/**
	 * Tail recursive solution to this problem. The complexity order in time and
